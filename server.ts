...
function writeDB(state: DatabaseState) {
  try {
    ...
    // Asynchronously push state updates to Supabase PostgreSQL Database in production
    if (supabase) {
      supabase.from("bizkhata_state")
        .upsert({ id: "default_ledger", state: state })
        .then(({ error }: any) => {
          if (error) {
            console.error("Async state sync with Supabase failed:", error);
          } else {
            console.debug("Database state successfully synchronized with Supabase in the background.");
          }
        })
        .catch((err: any) => {
          console.error("Internal error syncing with Supabase:", err);
        });
    }
  } catch (err) {
...
app.post("/api/user-seats/update", (req, res) => {
  const db = readDB();
...
(async () => {
  // 1. If Supabase is configured, pull initial ledger state from Supabase Cloud
  if (supabase) {
    try {
      console.log("Pulling initial ledger state from Supabase Cloud PostgreSQL...");
      ...
      if (error) {
        ...
        console.error("Supabase load failed with error status:", error.status, "code:", error.code, "message:", error.message, "details:", error.details, "hint:", error.hint);
        console.error("Please make sure you have executed the complete SQL schema defined in /supabase_schema.sql inside your Supabase SQL Editor. Also verify that Row Level Security (RLS) is either disabled or has proper policies configured.");
      } else if (data && data.state && Object.keys(data.state).length > 0) {
        supabaseStatus.connected = true;
        supabaseStatus.error = null;
        cachedDb = data.state;
        console.log("Successfully restored database state from Supabase Cloud version!");
      } else {
        supabaseStatus.connected = true;
        supabaseStatus.error = null;
        ...
        const init = getInitialState();
        cachedDb = init;
        // Seed initial state in Supabase asynchronously on startup
        await supabase.from("bizkhata_state").upsert({ id: "default_ledger", state: init });
        console.log("Successfully seeded default state inside Supabase CRM.");
      }
    } catch (err: any) {
      supabaseStatus.connected = false;
      supabaseStatus.error = { message: err?.message || String(err) };
      console.error("Failed to connect or fetch from Supabase on startup:", err);
    }
  }
