...
export default function Reports({ db, onTriggerAI, isLoadingAI, aiExplanation, onSaveChecklist }: ReportsProps) {
  ...
  const exportPDF = () => {
    ...
    frame.innerHTML = `<div style="text-align:center;margin-bottom:20px"><h2 style="font-size:16px;font-weight:900;margin:0">${selectedReport?.name}</h2><p style="color:#64748b;font-size:11px;margin:4px 0">${db.company.name} | ${db.company.gstin} | Period: ${fromDate} to ${toDate}</p></div><table style="width:100%;border-collapse:collapse">${tableRows.join("")}</table><div style="margin-top:20px;font-size:10px;color:#94a3b8;text-align:right">Generated by Ledgerio | ${new Date().toLocaleString("en-IN")}</div>`;
        ...
        <div className="w-full flex flex-col lg:flex-row gap-6">
          ...
          <main className="flex-1 bg-white border border-[#E5E1D8] rounded-2xl p-6 shadow-sm flex flex-col space-y-5">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center text-[10.5px] text-[#8C867A] pt-2 border-t border-[#E5E1D8] uppercase font-bold tracking-wider">
              <span>Ledgerio Enterprise Module Version 2026.1•87 Reports Total</span>
          ...
          <div className="bg-white border border-[#E5E1D8] rounded-2xl p-6 lg:col-span-2 space-y-6 flex flex-col justify-between shadow-sm min-h-[600px]">
            ...
            */}
            <div id="active-sheet-render-workspace" className="flex-1 py-3 text-xs">
              ...
              {selectedReport.id === "month_end_close" && (() => {
                ...
                const manualLabels: Record<string,string> = {
                  ...
                  payroll_entry: "Payroll journal posted (handled outside Ledgerio)",
                      ...
                      {manualSteps.map(s => (
                        <button key={s.key} onClick={() => toggleManualStep(s.key)} disabled={!onSaveChecklist}
                          ...
                          <span className="text-xs text-slate-700 flex items-center gap-2">
                            ...
                            {s.label}
                            {s.key === "payroll_entry" && <span className="text-[9px] text-slate-400 italic ml-1">(no payroll module in Ledgerio yet)</span>}
                          </span>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center text-[10.5px] text-[#8C867A] pt-4 border-t border-[#E5E1D8] uppercase font-bold tracking-wider mt-5 select-none">
              <span>Ledgerio Workspace Journal Sync: Verified</span>
