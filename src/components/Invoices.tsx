...
export default function Invoices({ db, onSaveInvoice, onIssueCreditNote, onAddCustomer, onTriggerAI, onSendInvoiceEmail, userRole, defaultTab }: InvoicesProps) {
  ...
  const itemsForCalc = formItems.filter(it => it.itemId !== "").map(it => ({
    ...
    // An unregistered business cannot legally charge GST — the line-item UI already
    // hides the rate selector and shows "No GST" in this case, but that was display-only;
    // the stored gstRate (defaulted to 18 on every new row) was never actually cleared,
    // so totals kept computing tax regardless. Zero it here as the authoritative guard.
    gstRate: orgIsGstRegistered ? it.gstRate : 0
  }));
  ...
  const handleInvoiceSubmit = async (e: React.FormEvent, overrideStatus?: 'Draft' | 'Sent' | 'Approved') => {
    ...
    const finalItems: InvoiceItem[] = formItems.map((fItem, idx) => {
      ...
      // Same guard as the live preview: an unregistered business cannot charge GST,
      // regardless of what gstRate happens to be stored on the item (defaults to 18).
      const effectiveGstRate = orgIsGstRegistered ? fItem.gstRate : 0;
      const lineAmount = fItem.qty * fItem.rate;
      const lineGst = (lineAmount * effectiveGstRate) / 100;
